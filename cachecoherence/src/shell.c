...
void rdump(FILE * dumpsim_file) {
  int k, i; 
    //~ printf("CURRENT_STATE[0].PC %" PRIx64 "\n", CURRENT_STATE[0].PC);
  ...
  for (k = 0; k < ARM_REGS - 1; k++)
    printf("X%d: 0x%" PRIx64 "\n", k, CURRENT_STATE[i].REGS[k]);
  printf("FLAG_N: %d\n", CURRENT_STATE[i].FLAG_N);
  printf("FLAG_Z: %d\n", CURRENT_STATE[i].FLAG_Z);
...
void get_command(FILE * dumpsim_file) {                         
  ...
  case 'i':
   if (scanf("%i %" PRIx64, &register_no, &register_value) != 2)
      break;
   CURRENT_STATE[current_CPU].REGS[register_no] = register_value;
   break;
...
void load_program(char *program_filename) {                   
  ...
  CURRENT_STATE[current_CPU].PC = MEM_TEXT_START;
